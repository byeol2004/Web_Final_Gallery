...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($check !== false) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
            // Store the photo URL like "https://example.com/uploads/image.jpg"
            $imageURL = "https://example.com/" . $targetFile;  // Replace with your base URL

            // Insert the photo data into the database
            $sql = "INSERT INTO photos (user_id, album_id, title, description, image_path) 
                    VALUES ('$userId', '$albumId', '$title', '$description', '$imageURL')";
